    ...
    async function searchCoins(query) {
        ...
        coinResults.innerHTML =
            '<div class="empty-state">Search failed. Try again.</div>';
        }
    }
    ...
    async function renderSearchResults() {
        ...
        allCoins.forEach((coin) => {
            ...
            coinEl.addEventListener("click", () => addToPortfolio(coin)); // FIXED: camelCase
    ...
    function renderPortfolio() {
        ...
        if (!portfolio.length) {
        portfolioList.innerHTML =
            '<div class="empty-state"><h3>Your portfolio is empty</h3><p>Add coins using search above!</p></div>';
        ...
        const pnlPercent = (
            (pnlValue / (item.amount * item.boughtPrice)) *
            100
        ).toFixed(2);
        ...
        portfolioEl.innerHTML = `
                    ...
                    pnlValue >= 0 ? "pnl-profit" : "pnl-loss"
                    }">
        ...
        coinCountDisplay.textContent = portfolio.length; 
    }
    ...
    searchInput.addEventListener("input", (e) => {
        ...
        searchTimeout = setTimeout(() => searchCoins(e.target.value), 500); // FIXED: setTimeout
        } else {
        coinResults.innerHTML = "";
        }
    });
